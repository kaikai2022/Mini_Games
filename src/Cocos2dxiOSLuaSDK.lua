...
---@public hideGodAdBannerView 隐藏googleADBanner
...
---@public showGodAdBannerView 显示隐藏googleADBanner
...
function Cocos2dxiOSLuaSDK:showVideo(callback)
    if (cc.PLATFORM_OS_ANDROID == targetPlatform) then

    end
    if (cc.PLATFORM_OS_IPHONE == targetPlatform) or (cc.PLATFORM_OS_IPAD == targetPlatform) then
        local args = { functionId = function(rule)
            if rule and rule == "true" and callback then
                callback(true)
            else
                if callback then
                    callback(false)
        ...
        end, } --参数只能传字典或者不传,此处传入的回调函数并非函数而只是个标识符

        local luaoc = require "cocos.cocos2d.luaoc"
        local ok, ret = luaoc.callStaticMethod("GoogleAdModSDK", "showVideo", args)
        if not ok then
            print("luaoc showDetailPage error:" .. ret)
        end
    end
